...
public class PlayerController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (rampaging && Vector3.Distance(transform.position, darkerHole.position) < rampageRange)
        {
        ...
        if(rampaging && Vector3.Distance(transform.position, darkerHole.position) < .2 && hasSucked)
        {
            hasSucked = false;
            fadeToBlack.SetActive(true);
            CameraShake.Instance.ShakeCamera(3, 5f);
    ...
    void ProcessInputs()
    {
        ...
        {
            eye.transform.localEulerAngles = new Vector4(0, (transform.position.y - darkerHole.position.y), -(transform.position.x - darkerHole.position.x), (transform.position.y - darkerHole.position.y)).normalized * 30;
            
        }

    }
    private IEnumerator FadeToBlack(float waitTime)
    {
